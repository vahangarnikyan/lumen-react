...
class AuthController extends Controller
{
    ...
    {
        $post1 = Post::where('slug', 'home_page')->where('post_type', 'post')->first();
        $post2 = Post::where('slug', 'home_part')->where('post_type', 'post')->first();

        $social = Options::getOption('social-medias', []);
    ...
    public function loginPost(Request $request)
    {
        ...
        if ($user) {
            if (Hash::check($request->input('password'), $user->password)) {
                ...
                $redirect = $request->get('redirect', Utils::localeUrl('/'));
